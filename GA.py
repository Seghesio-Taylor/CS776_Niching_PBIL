"""Genetic algorithm with restricted mating and niching."""
...
class GeneticAlgorithm:
    """Simple GA with tournament selection, crossover, mutation and niching.

    The algorithm evolves a population of binary chromosomes and applies
    restricted mating based on phenotypic distance.  Optional parameters
    control population size, generation count and operator probabilities.
    """
    ...
    def __init__(
        ...
        # Choose default sigma_mating based on function dimensionality
        ...
        # Determine decoding functions and distance measures
        if self.func_id in ('F1', 'F2'):
    ...
    def _evaluate_population(self, population: np.ndarray) -> np.ndarray:
        """Return an array of fitness values for a population."""
    ...
    def _tournament_select(self, fitness: np.ndarray) -> int:
        """Return an index via a small tournament."""
    ...
    def _restricted_tournament_replacement(
        ...
        """Restricted tournament replacement: offspring replaces a similar individual."""

        pop_size = population.shape[0]
        if pop_size == 0:
            return

        # ensure window is not larger than population
        w = min(window_size, pop_size)
        indices = self.rng.choice(pop_size, size=w, replace=False)

        # decode offspring phenotype
        off_pheno = self.decode(offspring)

        # compute distance to each candidate in the window
        distances = []
        for idx in indices:
            cand_pheno = decoded_phenotypes[idx]
            distances.append(self.distance(off_pheno, cand_pheno))

        # pick the closest individual in this window
        replace_idx = int(indices[int(np.argmin(distances))])

        # replace genotype and update phenotype list
    ...
    def _select_parent2(self, parent1_idx: int, population: np.ndarray, fitness: np.ndarray,
        ...
        """Pick a second parent based on phenotypic distance to the first."""
        # Compute distances from parent1 to all other individuals
        ...
        # Build neighbourhood: indices with distance < sigma_mating (exclude self)
        neighbours = [i for i in range(self.population_size)
                      if i != parent1_idx and dists[i] < self.sigma_mating]
        if neighbours:
            # Select parent2 among neighbours via tournament
            # Build a local tournament of size min(tournament_size, len(neighbours))
            k = min(self.tournament_size, len(neighbours))
            contenders = self.rng.choice(neighbours, size=k, replace=False)
            # Choose the best by fitness
            best_idx = contenders[0]
            best_fit = fitness[best_idx]
            for idx in contenders[1:]:
            ...
            # Fallback: choose the closest individual or a random one
            # Option: choose the closest phenotypically
            # Exclude self
    ...
    def _crossover_mutate(self, parent1: np.ndarray, parent2: np.ndarray) -> np.ndarray:
        """Return an offspring by one‑point crossover and bit‑flip mutation."""
        child = parent1.copy()
        # Crossover
        if self.rng.random() < self.crossover_prob:
            # Choose a crossover point between 1 and length-1
            cp = self.rng.integers(1, parent1.shape[0])
            # Take parent1 bits before cp and parent2 bits after cp
            child[:cp] = parent1[:cp]
            child[cp:] = parent2[cp:]
        # Mutation: flip bits with given probability
        mutation_mask = self.rng.random(child.shape) < self.mutation_prob
        # XOR with mutation_mask
        child = np.bitwise_xor(child, mutation_mask.astype(np.int8))
        return child

    def run(self) -> Dict[str, Any]:
        """Execute the genetic algorithm and return logs and final population data.
        ...
        dict
            ...
            - 'final_population': array of final chromosomes (shape (pop_size,30))
            - 'final_decoded': list of decoded phenotypes for final population
            - 'final_fitness': list of fitness values for final population
        """
        # Initialize population with random 0/1 bits
        ...
        for gen in range(self.generations):
            # Decode population phenotypes and evaluate fitness
            ...
            # --- RTR + elitism next generation ---
            # Start from current population, then modify via RTR
            new_population = population.copy()

            # Elitism: keep the best individual in slot 0
            new_population[0] = population[best_idx].copy()
            decoded[0] = decoded[best_idx]

            # Number of offspring to generate (we already kept one elite)
            num_offspring = self.population_size - 1

            for _ in range(num_offspring):
                # Select parents (parent1 with tournament, parent2 with restricted mating)
                ...
                # Restricted Tournament Replacement: child replaces a similar individual
        ...
        # Evaluate final population one last time
        decoded = [self.decode(ind) for ind in population]
        fitness = self._evaluate_population(population)
        return {
...
def run_ga(
    ...
    """Return a list of GA run results across ``num_runs`` experiments."""
...
def prepare_run_directory(model_name: str, func_id: str) -> Path:
    """Create and return a new ``run_n`` directory for a GA experiment."""
...
def plot_spaghetti_best_fitness(results: List[Dict[str, Any]], func_id: str, out_path: Path) -> None:
    """Plot spaghetti of best fitness per generation over multiple runs."""
    plt.figure()
...
def plot_1d_function_with_points(func_id: str, results: List[Dict[str, Any]], out_path: Path) -> None:
    """Plot 1D F1/F2 with final decoded points from all runs."""
    if func_id.upper() == "F1":
        f = bm.f1
    ...
    # collect final decoded phenotypes from last generation of each run
    finals = []
    for res in results:
        finals.extend(res["final_decoded"])
    # decode may return floats already; ensure 1D
...
def plot_himmelblau_with_points(results: List[Dict[str, Any]], out_path: Path) -> None:
    """Contour plot of Himmelblau + final GA individuals."""
    # grid
...
def save_ga_results_to_files(
    ...
    """Save per-run CSV + JSON summaries for GA runs."""
    # one CSV per run with best fitness per generation
...
def _ga_collect_all_points(results, func_id: str):
    """Collect all final phenotypes across all GA runs."""
    pts = []
    if func_id in ("F1", "F2"):
        for res in results:
            pts.extend(res["final_decoded"])  # list of floats
    else:
        for res in results:
            pts.extend(res["final_decoded"])  # list of (x1,x2)
    return pts

def ga_plot_peak_representatives(results, func_id: str, out_path: Path) -> None:
    """
    For GA: show only the best individual per peak, overlaid on f(x).
    """
    ...
    if func_id == "F1":
        ...
        # F3 is already a fitness (negative Himmelblau)
        def f_pair(p):
            return bm.f3(p[0], p[1])

    # Track best per peak
    best_per_peak = {}  # peak_idx -> (fitness, phenotype)
...
def ga_plot_peak_occupancy(results, func_id: str, out_path: Path) -> None:
    """
    For GA: bar plot of how many final individuals fall into each peak.
    """
    func_id = func_id.upper()
    pts = _ga_collect_all_points(results, func_id)
...
def _ga_best_run_index(results):
    """
    Choose the best GA run by final best fitness.
    """
    best_idx = 0
    best_val = -np.inf
    for i, res in enumerate(results):
...
def ga_plot_single_run_final(results, func_id: str, out_path: Path) -> None:
    """
    Plot final individuals from the single best GA run.
    """
    func_id = func_id.upper()
    ...
    if func_id == "F1":
        ...
        # F3 is already fitness (negative Himmelblau)
        def f_pair(p):
            return bm.f3(p[0], p[1])
    ...
    if func_id in ("F1", "F2"):
        ...
        # F3: contour + final individuals of best run
        x = np.linspace(-6, 6, 200)
        y = np.linspace(-6, 6, 200)
        X, Y = np.meshgrid(x, y)
        Z = np.zeros_like(X)
...
def main():
    ...
    results = run_ga(
        ...
        crossover_prob=0.9,
        mutation_prob=0.001,
        sigma_mating=None,  # use defaults 0.1 (F1/F2) or 1.0 (F3)
        tournament_size=2,
        seed=args.seed,
    )
